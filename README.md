...
## итоговый чек-лист для второго семинара (МДК.09.01). 
...
Это методическое руководство для третьего, завершающего семинара дня. 

---

### МЕТОДИЧЕСКОЕ РУКОВОДСТВО К СЕМИНАРУ №3
**Дисциплина:** МДК.09.03 Обеспечение безопасности веб-приложений
**Тема:** Защита от XSS-атак (Межсайтовый скриптинг)
**Дата:** 22 января 2026 г.
**Время:** 18:30 – 20:00
...
На прошлом занятии вы научились выводить данные из базы на экран. Но что, если пользователь (или злоумышленник) вместо названия товара напишет программный код?

**XSS (Cross-Site Scripting)** — это атака, при которой браузер выполняет вредоносный JavaScript-код, думая, что это часть вашего сайта.
*   **Последствия:** Кража кук (сессий), перенаправление на фишинговые сайты, изменение внешнего вида страницы.

**Ваша задача:** Сначала «взломать» свой сайт, а потом защитить его.

---

#### ЭТАП 1. ЛАБОРАТОРНАЯ РАБОТА «САМ СЕБЕ ХАКЕР» (30 минут)

**Цель:** Убедиться, что ваш сайт уязвим.

1.  Зайдите на свой сайт под **Администратором**.
2.  Перейдите на страницу добавления товара (или книги/заявки) — `add_item.php`.
3.  **Атака №1 (Дефейс/Внешний вид):**
    *   В поле «Название» введите: `<h1>Я ВЗЛОМАЛ ЭТОТ САЙТ</h1>`
    *   Нажмите «Сохранить».
    *   Перейдите на Главную.
    *   *Результат:* Если заголовок стал огромным и жирным — **уязвимость есть**. Браузер воспринял это как HTML-код.

4.  **Атака №2 (Выполнение JS):**
    *   Добавьте новый товар.
    *   В поле «Название» или «Описание» введите: `<script>alert("Ваши пароли украдены!");</script>`
    *   Нажмите «Сохранить».
    *   Перейдите на Главную.
    *   *Результат:* Если всплыло окошко с сообщением — **это критическая дыра**. Если бы там был код кражи кук, вы бы уже потеряли доступ к аккаунту.

---

#### ЭТАП 2. ИСПРАВЛЕНИЕ УЯЗВИМОСТИ (Fixing) (30 минут)

**Теория:** Нам нужно превратить спецсимволы HTML (`<`, `>`, `"`, `'`) в безопасные сущности (`&lt;`, `&gt;`). Тогда браузер просто покажет их как текст, но не будет выполнять.

В PHP для этого есть функция: `htmlspecialchars()`.

**Инструкция:**

1.  Откройте файл **`db.php`** (так как он подключен везде, это лучшее место для общей функции).
2.  В самом низу файла (перед закрывающей скобкой `?>`, если она есть) добавьте функцию-помощник `h()`:
...
3.  Теперь откройте файл **`index.php`** (Главная страница).
4.  Найдите места, где вы выводите данные. Обычно это выглядит так: `<?= $product['title'] ?>` или `echo $product['title']`.
5.  **Оберните все переменные в функцию `h()`**.
...
6.  **Проверка:**
    ...
    *   Вместо огромного заголовка вы должны увидеть просто текст `<h1>Я ВЗЛОМАЛ...</h1>`.
    *   Всплывающее окно `alert` больше не должно появляться. Вы должны видеть сам код скрипта текстом.

**Вывод:** Теперь любые данные, введенные пользователем, безопасны для отображения.

---

#### ЭТАП 3. ПОДВЕДЕНИЕ ИТОГОВ ДНЯ (Code Review)

Перед тем как уйти домой, убедитесь, что ваш проект соответствует трем критериям оценки преподавателя (Ивлев А.Л.).

**Чек-лист готовности (Self-Check):**

1.  **[ ] Разделение прав (RBAC):**
    *   Есть файл `check_admin.php`.
    *   Если выйти из аккаунта и попытаться открыть `add_item.php` (или `admin_panel.php`) по прямой ссылке — сайт не пускает.

2.  **[ ] Работа с Базой Данных:**
    *   Создана таблица под вашу тему (не только `users`).
    *   Вы можете добавлять записи через форму, и они сохраняются в БД.
    *   Записи выводятся на главной странице.

3.  **[ ] Безопасность (XSS):**
    *   В коде вывода (`index.php`) везде используется функция `h()` или `htmlspecialchars()`.
    *   HTML-теги в названиях товаров отображаются как текст, а не как разметка.

---

### ПРИЛОЖЕНИЕ: ШПАРГАЛКИ ПО SQL (ДЛЯ ТЕХ, КТО ОТСТАЛ)

Если вы еще не создали таблицу для своей темы на 2-й паре, используйте эти шаблоны. Выполнять в **phpMyAdmin -> SQL**.

**Вариант 1. Для темы «Сервис обмена вещами» (Тема 22):**
```sql
CREATE TABLE `items` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `title` VARCHAR(255) NOT NULL COMMENT 'Название вещи',
    `description` TEXT COMMENT 'Описание',
    `contact_info` VARCHAR(255) NOT NULL COMMENT 'Как связаться',
    `status` ENUM('active', 'exchanged') DEFAULT 'active',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP
);
```

**Вариант 2. Для темы «HelpDesk / Техподдержка» (Тема 12):**
```sql
CREATE TABLE `tickets` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `subject` VARCHAR(255) NOT NULL COMMENT 'Тема проблемы',
    `message` TEXT NOT NULL COMMENT 'Суть проблемы',
...
**Вариант 3. Для темы «Блог / Новости» (Тема 20):**
```sql
CREATE TABLE `posts` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
...
# ДОМАШНЕЕ ЗАДАНИЕ (К СЛЕДУЮЩЕЙ СЕРИИ СЕМИНАРОВ)
**Цель:** Закрепить навыки работы с БД и подготовить базу для реализации функционала "Личный кабинет" и "Редактирование/Удаление".

#### ЗАДАНИЕ 1. Расширение Базы Данных (SQL)
Вам необходимо добавить в таблицу вашей основной сущности (`products`, `tickets`, `posts`) поле, которое свяжет запись с конкретным пользователем.
1.  Зайдите в phpMyAdmin.
2.  Добавьте в вашу таблицу поле `user_id` (тип INT, индекс INDEX).
3.  *(Для продвинутых)* Настройте внешний ключ (Foreign Key) на таблицу `users`.

#### ЗАДАНИЕ 2. Персонализация (PHP)
Модифицируйте файл `add_item.php`:
1.  При сохранении записи в БД, в поле `user_id` записывайте ID текущего пользователя из сессии (`$_SESSION['user_id']`).
    *   *Подсказка:* Это нужно добавить в `INSERT` запрос.

#### ЗАДАНИЕ 3. Самостоятельное изучение (МДК.09.02)
Изучите тему **"Пагинация (Постраничный вывод) в PHP/MySQL"**.
*   **Вопрос:** Если товаров будет 1000, как вывести их не все сразу, а по 10 штук на странице?
*   **Ключевые слова для поиска:** `SQL LIMIT OFFSET`, `PHP pagination logic`.
*   *Задача:* Подготовить краткий конспект (или схему) того, как это реализуется.

#### ЗАДАНИЕ 4. Подготовка контента
Найдите и сохраните 5-10 качественных изображений для вашего проекта. Оптимизируйте их (сжатие через tinypng.com), чтобы они весили не более 200 Кб. Это пригодится для занятия по оптимизации.

---
**Срок сдачи:** Начало следующего занятия (26.01.2026).
**Форма контроля:** Демонстрация работающего кода на хостинге Beget.
